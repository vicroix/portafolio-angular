...
export class DetailsComponent implements OnInit {
  constructor(private enviarRutasImgServ: AbrirModalImgService){}
  @Input() fontH2 = '';
  botonGit1 =
    'flex gap-3 enlace-git justify-center items-center bg-[rgb(176,62,255)] text-white hover:bg-[rgb(189,95,255)] cursor-pointer active:bg-[rgb(166,91,219)] w-35 py-2 shadow-sm shadow-gray-500 rounded-xl';
  ...
  modal() {
    this.enviarRutasImgServ.emit(this.rutasImgModal);
  }
  cargarProyecto(id: string | null): void {
    this.http.get<any[]>('/assets/proyectos.json').subscribe((proyectos) => {
      ...
      this.rutasImgModal = this.proyecto.imagen;
